...
<template>
  ...
  <section class="posts" v-if="loaded && posts.length !== 0">
    <article v-for="post in posts" :key="post._id">
      <PostComponent v-if="editing !== post._id" :post="post" @refreshPosts="() => getPosts(searchWord)" @editPost="updateEditing" />
      <EditPostForm v-else :post="post" @refreshPosts="() => getPosts(searchWord)" @editPost="updateEditing" />
    </article>
  </section>
